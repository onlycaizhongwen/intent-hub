...
- [v] 补运维样例总入口。
  - 当前产物：`ops/README.md`，按 Prometheus scrape、告警规则、Alertmanager 路由、Grafana 看板、SLO、本地观测栈和 Runbook 串联接入顺序，并同步 README/status/HTML/trace。
  - 下一步：已完成；后续生产化从该入口补真实服务发现、鉴权、receiver、持久化、SLO 审批和演练流程。
- [v] 补生产化落地检查清单。
  - 当前产物：`ops/production-readiness-checklist.md`，按指标入口、Prometheus、Alertmanager、Grafana、SLO、安全合规、演练验收拆解生产化必改项。
  - 下一步：已完成；后续选择试点环境逐项打勾，并把真实值班通道、权限、TLS 和演练证据回填。
- [v] 补观测告警试点接入计划。
...
- [v] 补告警演练场景。
  - 当前产物：`ops/alert-drill-scenarios.md`，覆盖 6 条 P2.x 告警的触发目标、推荐触发方式、验证点、恢复方式和禁止动作。
  - 下一步：已完成；试点 Day 6 按该文档执行并回填记录模板。
- [v] 补本地观测栈预检脚本。
  - 当前产物：`scripts/check-observability-local.ps1`，检查本地 ops 配置文件、Intent Hub health/metrics endpoint 和 Docker 命令。
  - 下一步：已完成；本地启动 compose 前先执行该脚本。
...
## 2026-06-04 补充记录：运维样例总入口

- 本轮目标：在 Prometheus、Alertmanager、Grafana、SLO、本地观测栈和 Runbook 分散样例完成后，补一个统一阅读与接入入口。
- 已完成：新增 `ops/README.md`，并同步 `README.md`、`docs/codex/v1/status.md`、HTML 生命周期页、P2-3 指标观测 trace 和 P2-5 LLM 治理 trace。
- 接入口径：按 Prometheus scrape -> Prometheus 告警规则 -> Alertmanager 路由 -> Grafana 看板 -> SLO -> 本地观测栈 -> Runbook 排列。
- 边界：该入口仍是样例导航，不是生产部署方案；真实生产需补服务发现、TLS/鉴权、真实 receiver、持久化、高可用、正式 SLA 审批和演练流程。

## 2026-06-04 补充记录：生产化落地检查清单

- 本轮目标：把运维样例进入真实环境前的必改项整理为可逐项确认的 checklist。
- 已完成：新增 `ops/production-readiness-checklist.md`，并同步 `ops/README.md`、`docs/codex/v1/status.md`、HTML 生命周期页、P2-3 指标观测 trace 和 P2-5 LLM 治理 trace。
- 检查口径：指标入口、Prometheus、Alertmanager、Grafana、SLO 与错误预算、安全合规、演练与验收。
- 边界：清单只是准入门槛和确认项，不代表真实生产环境已经完成部署、鉴权、TLS、receiver、SLO 审批或演练。

## 2026-06-04 补充记录：观测告警试点接入计划

- 本轮目标：把生产化检查清单拆成 dev/staging 试点环境可执行的一周接入计划。
...
## 2026-06-04 补充记录：告警演练场景

- 本轮目标：为 Day 6 告警演练补充可执行场景，避免只知道要演练但不知道如何安全触发。
- 已完成：新增 `ops/alert-drill-scenarios.md`，并同步 `ops/README.md`、`ops/pilot-rollout-plan.md`、`ops/pilot-execution-record-template.md`、status、HTML 生命周期页、P2-3 指标观测 trace 和 P2-5 LLM 治理 trace。
- 演练口径：6 条 P2.x 告警的触发目标、推荐触发方式、验证点、恢复方式和禁止动作。
- 边界：文档只提供安全演练方案，不代表真实环境已经完成告警触发、通知或恢复验证。

## 2026-06-04 补充记录：本地观测栈预检脚本

- 本轮目标：在启动本地 Docker Compose 观测栈前，先提供可重复的环境和文件预检。
- 已完成：新增 `scripts/check-observability-local.ps1`，并同步 `ops/local-observability/README.md`、`ops/README.md`、status、HTML 生命周期页、P2-3 指标观测 trace 和 P2-5 LLM 治理 trace。
- 检查口径：ops 本地观测栈配置文件、Prometheus/Grafana 样例文件、Intent Hub health endpoint、metrics endpoint 和 Docker 命令。
- 边界：脚本只做预检，不启动 compose，不修改配置；当前未要求真实启动 Docker Compose。
