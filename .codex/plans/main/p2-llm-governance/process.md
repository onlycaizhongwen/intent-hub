...
- 当前阶段：P2.x 日预算门禁已完成；本轮在既有 LLM 受控兜底基础上补齐外呼前日预算最小门禁和管理端预算查询，相关模块测试通过，待用户明确指令后推送 GitHub。
...
- [v] 补 LLM 预算持久化审计。
  ...
  - 下一步：已完成；日预算最小门禁和管理端查询已补，后续做事务级强扣减、跨实例并发保护和告警。
...
- 当前 P2-5 还不是完整生产 LLM：已预接入 Spring AI Alibaba `ChatClient`，但没有真实 DashScope 沙箱冒烟和跨实例强配额扣减；底层 HTTP timeout 绑定、最小预算消费计数、持久化审计、日预算最小门禁和管理端查询已完成。
