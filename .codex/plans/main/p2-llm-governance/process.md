...
- [v] 补本地观测栈配置校验脚本。
  - 当前产物：`scripts/validate-observability-compose.ps1`，不启动容器校验 Docker Compose 文件、Prometheus rule/target 引用和 Grafana provisioning 引用。
  - 下一步：已完成；本地启动 compose 前先执行 validate 脚本，再执行 preflight 脚本。
...
## 2026-06-04 补充记录：本地观测栈配置校验脚本

- 本轮目标：在不启动容器的情况下，提前发现 Docker Compose、Prometheus 和 Grafana provisioning 引用错误。
