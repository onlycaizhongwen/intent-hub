...
- 当前阶段：P2.x 日预算原子预占已完成；本轮在既有 LLM 受控兜底基础上补齐外呼前日预算原子预占和管理端预算查询，相关模块测试通过，待用户明确指令后推送 GitHub。
...
- [v] 补 LLM 预算消费最小计数。
  ...
  - 下一步：已完成；持久化审计已补，日预算原子预占已补，后续再做跨实例失败补偿、预算释放和告警。
- [v] 补 LLM 预算持久化审计。
  ...
  - 下一步：已完成；日预算原子预占和管理端查询已补，后续做跨实例失败补偿、预算释放和告警。
...
- 当前 P2-5 还不是完整生产 LLM：已预接入 Spring AI Alibaba `ChatClient`，但没有真实 DashScope 沙箱冒烟和跨实例失败补偿、预算释放和告警；底层 HTTP timeout 绑定、最小预算消费计数、持久化审计、日预算原子预占和管理端查询已完成。
...
- 2026-06-03：将 LLM 日预算门禁从“查询后记账”升级为外呼前原子预占：`LlmBudgetAuditPort.tryReserveDailyBudget` 负责按有效日预算预占；memory 实现用同步计数保护，JDBC 实现用预算保留行、唯一键和条件 update 控制超额；日用量查询排除预算保留行，避免预占与明细审计双算。`mvn -pl intent-hub-infrastructure,intent-hub-interfaces -am test` 通过，相关模块共 52 个测试。
