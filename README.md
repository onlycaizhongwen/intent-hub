...
当前验证结果：`mvn test` 通过，全量共 54 个测试；`mvn package -DskipTests` 通过。
...
- 基础设施层已验证治理关闭、策略预算为 0、日预算耗尽、全局预算收紧、成功返回候选、有限重试后失败，以及 memory/JDBC 日预算预占不双算且可暴露 pending 差额等场景。
...
- P2.x：已完成 LLM adapter 的 Spring AI Alibaba `ChatClient` 预接入、DashScope 沙箱冒烟脚本准备、日预算原子预占门禁和管理端 confirmed/reserved/pending 查询；下一步在提供沙箱凭证后补真实 trace/metrics/bad case 证据，并补跨实例失败补偿、预算释放和告警。
