...
| 接口 | 方法 | 说明 |
| --- | --- | --- |
| `/api/v1/admin/config/versions/{version}/{objectType}` | POST | 新增或更新配置对象 |
| `/api/v1/admin/config/versions/{version}/{objectType}` | GET | 查询配置对象列表 |
| `/api/v1/admin/config/versions/{version}/{objectType}/bulk` | POST | 批量导入配置对象 |
| `/api/v1/admin/config/versions/{version}/{objectType}/{objectId}` | DELETE | 删除配置对象 |
...
当前 P1-2 已完成，P1-3 PostgreSQL/Flyway 持久化闭环已通过真实库联调，P1-4 配置治理 API 版本生命周期、JDBC 联调、配置对象 CRUD/批量导入/删除、配置字段基础校验、发布前跨对象引用校验、已发布配置读取、配置版本审计查询、`scene_routing_rule.match_condition` 最小后置路由条件解析和显式 `actionSchema.intentCode` 动作归属读取已完成，P1-5 可观测与数据回流最小查询闭环已完成，P1-6 退出评审已通过。P2-1 已补齐动态 scene 读取最小闭环，P2-2 已补齐 bad case 标注、关闭和训练样本导出最小闭环，P2-3 已补齐最小指标采集与观测接口，P2-4 已补齐模型服务 adapter 最小闭环，P2-5 已补齐 LLM 受控兜底最小闭环；后续已补齐模型服务/LLM HTTP timeout 绑定、模型服务异常失败关闭、fallback 指标口径、LLM 预算消费最小计数、持久化审计、日预算原子预占门禁、管理端 confirmed/reserved/pending 查询、同步失败释放、stale pending 后台补偿、模型 adapter 本地 HTTP 冒烟、FastAPI 模型服务示例工程、模型服务健康检查、本地真实联调、scene 级模型 endpoint/timeout 动态路由、endpoint + timeout 维度的模型客户端缓存复用、`authTokenRef` 模型服务 Bearer 鉴权头注入和 token 引用缺失失败关闭。下一步建议进入真实 DashScope 沙箱冒烟、真实多实例 PostgreSQL 压测和生产级模型服务连接池治理/鉴权联调。
...
- 配置对象最小 CRUD 已完成：`POST/GET/DELETE /api/v1/admin/config/versions/{version}/{objectType}` 支持意图、槽位、同义词、策略、路由、下游动作；`POST /bulk` 支持同类型对象批量导入。
...
- `mvn test` 通过，共 71 个测试。

当前边界：

- 第一阶段只完成版本生命周期和导入导出边界。
- 配置对象删除、批量导入、基础字段细校验、发布前跨对象引用校验、POST 后置路由条件解析和显式 intent/action 关系读取已完成；当前已拦截 `confidenceThreshold` / `modelPolicy.minConfidence` 范围、`routeStage` 枚举、`actionType` 白名单、`timeoutMs` 合理区间、`llmPolicy.dailyBudget/maxRetries/timeoutMs` 等高风险字段，并在发布前检查 slot 所属 intent、POST route 下游动作、downstream action 显式或兼容推断 intent 的最小引用完整性。
- 前置路由动态 scene 读取已在 P2-1 完成最小闭环：JDBC 已发布配置读取不再固定 `order-scene`，支持 metadata 显式 scene、租户最新发布 scene 和内置配置回退。

后置路由条件解析：

- 已支持 `scene_routing_rule.match_condition` 的 P1/P2 最小规则化解析，不引入完整表达式引擎。
- 支持 `intentCode` / `intent_code` 匹配候选意图。
- 支持 `minConfidence` / `min_confidence` 匹配最低置信度。
- 支持 `slots` / `slotEquals` / `slotConditions` / `slot_conditions` 等槽位等值条件。
- POST 规则按 `priority asc, id asc` 读取，先命中先选择；未命中条件规则时回退到原 intent -> downstream action 映射。
- `route_target` 可直接指向 `downstream_action.action_code`；仓储优先读取 `downstream_action.action_schema.intentCode` 建立动作归属意图索引，旧配置继续保留按动作编码后缀推断 intent 的兼容索引。
...
- `ModelClientAdapterTest` 覆盖 no-op adapter、inactive HTTP adapter、MockRestServiceServer 成功返回、模型服务健康检查、JDK 本地 HTTP server 冒烟、scene endpoint 覆盖全局 base-url、无全局 base-url 时使用 scene endpoint、endpoint + timeout 缓存复用、`authTokenRef` 解析后的 Bearer 鉴权头注入，以及 token 引用缺失时不外呼。
...
- 模型服务全局 `enabled=false` 时仍保持 no-op；全局开启后，运行时可优先使用 scene 级 `modelPolicy.endpoint/timeoutMs` 覆盖或补齐模型服务请求地址与超时，并按 endpoint + timeout 复用场景客户端。
- `timeoutMs` 已通过 `SimpleClientHttpRequestFactory` 绑定到模型服务 HTTP adapter 的 connect/read timeout；scene 级 timeout 会覆盖全局 timeout。
- `modelPolicy.authTokenRef` 只保存 token 引用名，运行时从系统属性或环境变量解析后注入 `Authorization: Bearer ...`；不在 DB、文档或代码中保存明文 token。若引用名存在但 token 无法解析，识别链路记录 `MODEL_FALLBACK:AUTH_MISSING_TOKEN` 并失败关闭，避免向模型服务发无鉴权请求。
