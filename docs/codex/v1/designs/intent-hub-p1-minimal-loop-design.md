...
当前 P1-2 已完成，P1-3 PostgreSQL/Flyway 持久化闭环已通过真实库联调，P1-4 配置治理 API 版本生命周期、JDBC 联调、配置对象最小 CRUD 与已发布配置读取已完成，P1-5 可观测与数据回流最小查询闭环已完成。下一步建议进入 P1-6 退出评审，同时补配置治理细化、指标采集和 bad case 标注流转。
...
- 前置路由动态 scene 选择尚未完成，当前已发布配置读取仍以 `order-scene` 为 P1 最小试点。

下一步：

- 补配置对象删除、批量导入和更细字段校验。
- 补前置路由动态 scene 选择和更多已发布配置读取场景。
- 推进 P1-6 退出评审，并补 Prometheus/OpenTelemetry 指标与 bad case 标注状态流转。
