...
- 配置对象删除、批量导入、基础字段细校验、发布前跨对象引用校验和 POST 后置路由条件解析已完成；当前已拦截 `confidenceThreshold` / `modelPolicy.minConfidence` 范围、`routeStage` 枚举、`actionType` 白名单、`timeoutMs` 合理区间、`llmPolicy.dailyBudget/maxRetries/timeoutMs` 等高风险字段，并在发布前检查 slot 所属 intent、POST route 下游动作、downstream action 反推 intent 的最小引用完整性。
...
- `route_target` 可直接指向 `downstream_action.action_code`；仓储同时保留按动作编码后缀推断 intent 的兼容索引。

下一步：

- 后续可继续增强显式 intent/action 关系字段，替代当前按 `ACTION_CODE` 后缀反推 intent 的兼容策略。
